...
namespace Aop.Api.Util.Asymmetric
{
	...
	/// 签名部分采用SHA1算法进行摘要计算
	/// </summary>
	public class RSAEncryptorV2 : BaseAsymmetricEncryptor
	{
		private static readonly ConcurrentDictionary<string, RSACryptoServiceProvider> rsaCache = new ConcurrentDictionary<string, RSACryptoServiceProvider>();
		...
		{
			if (!rsaCache.TryGetValue(privateKey, out RSACryptoServiceProvider rsaService))
			{
				rsaService = rsaCache.GetOrAdd(privateKey, BuildRSAServiceProvider(Convert.FromBase64String(privateKey)));
			}
		...
		{
			if (!rsaCache.TryGetValue(publicKey, out RSACryptoServiceProvider rsaService))
			{
				rsaService = rsaCache.GetOrAdd(publicKey, BuildRSAServiceProvider(Convert.FromBase64String(publicKey)));
			}
